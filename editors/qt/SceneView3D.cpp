...
void SceneView3D::mouseDoubleClickEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton) {
        QString entityName = getEntityAt(event->pos());
        
        if (!entityName.isEmpty()) {
            // Двойной клик по объекту - фокусируемся на нем
            focusOnEntity(entityName);
            qDebug() << "Double clicked entity:" << entityName;
        }
    }
}
...
void SceneView3D::focusOnEntity(const QString &name)
{
    for (const auto &entity : m_entities) {
        if (entity.name == name) {
            // Фокусируемся на объекте - перемещаем камеру к нему
            QVector3D targetPos = entity.position;
            QVector3D cameraPos = targetPos + QVector3D(0, 5, 5); // Смещаем камеру немного назад и вверх
...
void SceneView3D::setupShaders()
{
    ...
    qDebug() << "Main shader program linked successfully";
    
    // Шейдер для сетки
    m_gridShaderProgram = new QOpenGLShaderProgram(this);
    
    if (!m_gridShaderProgram->addShaderFromSourceCode(QOpenGLShader::Vertex, gridVertexShaderSource)) {
        qDebug() << "Grid vertex shader compilation failed:" << m_gridShaderProgram->log();
        return;
    }
    
    if (!m_gridShaderProgram->addShaderFromSourceCode(QOpenGLShader::Fragment, gridFragmentShaderSource)) {
        qDebug() << "Grid fragment shader compilation failed:" << m_gridShaderProgram->log();
        return;
    }
    
    if (!m_gridShaderProgram->link()) {
        qDebug() << "Grid shader program linking failed:" << m_gridShaderProgram->log();
        return;
    }
...
void SceneView3D::renderEntities()
{
    if (!m_shaderProgram || !m_shaderProgram->isLinked()) {
        qDebug() << "Shader program not available or not linked";
        return;
    }
...
{
    // Простая реализация определения объекта под курсором
    // В реальной реализации здесь должен быть ray casting
    
    // Нормализуем координаты экрана
    float x = (2.0f * screenPos.x()) / width() - 1.0f;
    float y = 1.0f - (2.0f * screenPos.y()) / height();
    
    // Простая проверка: находим ближайший объект к центру экрана
    QVector3D mouseWorldPos = screenToWorld(screenPos);
    
    float minDistance = std::numeric_limits<float>::max();
    QString closestEntity;
    ...
    for (const auto &entity : m_entities) {
        ...
        QVector3D entityPos = entity.position;
        float distance = (mouseWorldPos - entityPos).length();
        
        if (distance < minDistance && distance < 2.0f) { // 2.0f - радиус выбора
            minDistance = distance;
            closestEntity = entity.name;
        }
    }
